...
let utilFunctions = {
    ...
    dialog.showOpenDialog((fileNames) => {
      ...
      readStream.on('data', (codeEncrypt) => {
        try {
          ...
          utilFunctions.setModalError('', MESSAGES.modals().format, MESSAGES.modals().try_again);
        }
      });
    });
  },
      ...
      async function Devices() {
        try {
          ...
          if ((await FIRMATA.manageDevice('mega', device)) == 'Exito') {
            log(CHALK.gray.bgGreen.bold(MESSAGES.modals('MEGA').devices.success.config));
          } else {
            log(CHALK.gray.bgRed.bold(MESSAGES.modals('MEGA').devices.errors.config));
          }
          let resNano = await FIRMATA.manageDevice('nano', device);
          if (resNano == 'Exito') {
            log(CHALK.gray.bgGreen.bold(MESSAGES.modals('NANO').devices.success.config));
          } else {
  ...
  verifyInternet: (ipcRenderer, document) => {
    ...
    if (device) {
      ...
      INTERNET_AVAILABLE()
        .then(() => {
          log(CHALK.gray.bgGreen.bold(MESSAGES.modals().internet.success.blokino_connected));
  ...
  validateModal: (current_test, tests, complete_challenge) => {
    if (complete_challenge) {
      ...
      switch (current_test) {
        case 'test_1':
          utilFunctions.setModalSuccess(MESSAGES.modals('Prueba 1').challenges.success.test);
          $('#test1_title')
          ...
          break;
        case 'test_2':
          utilFunctions.setModalSuccess(MESSAGES.modals('Prueba 2').challenges.success.test);
          $('#test2_title')
          ...
          break;
        case 'test_3':
          utilFunctions.setModalSuccess(MESSAGES.modals('Prueba 3').challenges.success.test);
          $('#test3_title')
          ...
          break;
        case 'test_4':
          utilFunctions.setModalSuccess(MESSAGES.modals('Prueba 4').challenges.success.test);
          $('#test4_title')

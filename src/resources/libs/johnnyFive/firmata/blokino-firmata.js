...
const blokinoFirmata = {
  showSupported: () => {
    log(
      chalk.black.bgYellow.bold(`${FIRMATA_LABELS.KEYS.SUPPORT_DEVICES}${supportedBoardsString}`),
    );
  },
  manageDevice: (board, port) => {
    ...
    log(chalk.black.bgYellow.bold(FIRMATA_LABELS.SETUP.SETUP_BOARD + options.board));
  ...
  flash: (options, callback) => {
    ...
    const DIR = path.dirname(require.resolve(FIRMATA_LABELS.KEYS.AVR_ARDUINO));
    const FIRMATA_DIRECTORY = path.resolve(DIR, 'junk', 'hex', options.board);
    let firmataPath;
    fs.readdir(FIRMATA_DIRECTORY, (err, files) => {
      ...
      for (let i = 0, len = files.length; i < len; i++) {
        ...
        if (filename.indexOf(FIRMATA_LABELS.KEYS.STANDARD_FIRMATA) > -1) {
      ...
      if (typeof firmataPath === 'undefined') {
        return log(
          chalk.white.bgRed.bold(`${FIRMATA_LABELS.KEYS.FIRMATA_NOT_FOUND}${options.board}`),
        );
      }

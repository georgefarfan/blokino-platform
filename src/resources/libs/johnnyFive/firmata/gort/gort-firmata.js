...
const gortFirmata = {
  devices: async function (callback) {
    let devices = await serialportCommand.module.listDevices();
  ...
  gortSetup: function (device) {
    ...
    switch (os_arch) {
      case 'Linx86':
        sudo.exec(
          ...
          function (error, stdout, stderr) {
            if (error !== '') {
              ...
              log(chalk.white.bgGreen.bold('Dispositivo ', device, ': Arduino instalado.'));
            } else {
        ...
        sudo.exec(
          '' + path.resolve(__dirname) + '/gort_lin_x86 arduino upload firmata ' + device + '',
          options,
          function (error, stdout, stderr) {
            if (error !== '') {
              log(chalk.white.bgGreen.bold('FIRMATA respuesta: ' + stdout));
              log(chalk.white.bgGreen.bold('Dispositivo ', device, ': Firmata inatalado.'));
            } else {
              //Se presento un error
              log(chalk.black.bgRed.bold('FIRMATA error: ' + stderr));
            }
      ...
      case 'Linx64':
        ...
        sudo.exec(
          '' + path.resolve(__dirname) + '/gort_lin_x64 arduino upload firmata ' + device + '',
          options,
          function (error, stdout, stderr) {
            if (error !== '') {
              log(chalk.white.bgGreen.bold('FIRMATA respuesta: ' + stdout));

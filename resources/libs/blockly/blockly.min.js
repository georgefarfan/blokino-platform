...
Blockly.VariableMap.prototype.deleteVariableById=function(a){
  ...
  if(b){
    var c=b.name,d=this.getVariableUsesById(a);a=0;
    for(var e;e=d[a];a++)
      ...
      let msg = 1<d.length? Blockly.Msg.DELETE_VARIABLE_CONFIRMATION.replace("%1",String(d.length)).replace("%2",c): `¿Está seguro de eliminar la variable ${c}?`;
      
      Blockly.confirm({variable:c, type:"delete_variable",message: msg},function(a){ 
                a && f.deleteVariableInternal_(b,d)
      })
     
      /*
      1<d.length?
        ( c= Blockly.Msg.DELETE_VARIABLE_CONFIRMATION.replace("%1",String(d.length)).replace("%2",c),
            Blockly.confirm({variable:c, type:"delete_variable"},function(a){ 
                      a && f.deleteVariableInternal_(b,d)
            })
        ): (f.deleteVariableInternal_(b,d);Blockly.confirm({variable:c, type:"delete_variable"});)*/
  }
    else console.warn("Can't delete non-existent variable: "+a)
};
Blockly.VariableMap.prototype.deleteVariableInternal_=function(a,b){var c=Blockly.Events.getGroup();c||Blockly.Events.setGroup(!0);try{for(var d=0;d<b.length;d++)b[d].dispose(!0,!1);this.deleteVariable(a)}finally{c||Blockly.Events.setGroup(!1)}};
